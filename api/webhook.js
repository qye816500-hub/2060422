// api/webhook.js
// LINE Bot — 個人管理工具 v7.0.0
// 新增：bot state 機制、任意 URL 收藏、待辦直接輸入流程
...
const row = data.slice(1).find((r) => String(r[0] || “”) === userId);
...
const rowIndex = data.slice(1).findIndex((r) => String(r[0] || “”) === userId);
...
if (req.method === “GET”) {
return res.status(200).json({ status: “ok”, version: “7.0.0” });
}
...
height: “sm”,
flex: 1,
action: { type: “postback”, label: “查看待辦”, data: “action=todo_view”, displayText: “查看待辦” },
},
{
type: “button”,
style: “primary”,
...
flex: 1,
color: “#2D3561”,
action: { type: “postback”, label: “新增待辦”, data: “action=todo_new”, displayText: “新增待辦” },
},
],
},
{
...
height: “sm”,
flex: 1,
action: { type: “postback”, label: “完成待辦”, data: “action=todo_complete”, displayText: “完成待辦” },
},
{
type: “button”,
style: “secondary”,
height: “sm”,
flex: 1,
action: { type: “postback”, label: “刪除待辦”, data: “action=todo_delete”, displayText: “刪除待辦” },
},
],
},
],
...
// A: todoId | B: content | C: status | D: createdAt | E: remindText | F: userId | G: todo | H: googleEventId
const row = [id, content, “pending”, now, remindText, userId, “todo”, googleEventId];
await appendRow(sheets, SHEET.TODOS, row);

const timeInfo = remindText ? `\n⏰ 提醒時間：${remindText}` : “”;
const calInfo = googleEventId ? “\n📅 已同步到 Google 日曆” : “”;
...
…(remindText ? [makeRow(“提醒”, remindText)] : []),
…(googleEventId ? [{ type: “text”, text: “📅 已同步到 Google 日曆”, size: “xs”, color: “#0F9D58” }] : []),
],
},
footer: {
type: “box”,
...
flex: 1,
color: “#2D3561”,
action: { type: “postback”, label: “再新增一筆”, data: “action=todo_new”, displayText: “新增待辦” },
},
],
},
},
...
“目前沒有待辦”,
“點下方按鈕新增第一筆待辦”,
[{ label: “新增待辦”, data: “action=todo_new”, displayText: “新增待辦” }],
true
)
);
return;
...
const content = String(r[1] || “”);
const remind = String(r[4] || “”);
return remind ? `${i + 1}. ${content}（${remind}）` : `${i + 1}. ${content}`;
});

// 寫入 state，下一句直接輸入序號
// 這裡用 sheets 參數需從外層取得，改用文字提示方式
...
const googleEventId = String(target.row[7] || “”);
...
// 若有綁定 Google Calendar，同步刪除
// （需要 calendar client，這裡先標記刪除，之後擴充）
...
if (/linkedin.com/i.test(url)) return { name: “LinkedIn”, emoji: “💼”, color: “#0077B5” };
if (/github.com/i.test(url)) return { name: “GitHub”, emoji: “🐱”, color: “#333333” };
if (/notion.so/i.test(url)) return { name: “Notion”, emoji: “📝”, color: “#000000” };
if (/medium.com/i.test(url)) return { name: “Medium”, emoji: “📖”, color: “#000000” };
...
{ type: “text”, text: `${platform.emoji} ${platform.name}`, color: “#FFFFFFCC”, size: “sm” },
],
},
body: {
type: “box”,
...
await replyFlex(
replyToken,
buildEmptyStateFlex(“還沒有收藏”, “貼上任意連結就會自動收藏”, [{ label: “使用說明”, text: “說明” }])
);
return;
}
...
spacing: “lg”,
contents: [
menuItem(“🔗”, “貼連結收藏”, “任意網址自動收藏，可直接分類”),
{ type: “separator” },
menuItem(“📋”, “待辦清單”, “查看、新增、完成、刪除待辦”),
{ type: “separator” },
menuItem(“📁”, “傳圖片／檔案”, “自動備份到 Google Drive”),
{ type: “separator” },
menuItem(“📅”, “說待辦事項”, “含時間則新增到 Google 日曆”),
],
},
footer: {
type: “box”,
...
function buildEmptyStateFlex(title, description, buttons = [], isPostback = false) {
return {
type: “flex”,
altText: title,
contents: {
...
style: “secondary”,
height: “sm”,
action: { type: “postback”, label: b.label, data: b.data, displayText: b.displayText },
}
: {
type: “button”,
style: “secondary”,
