// api/webhook.js
// LINE Bot - v7.1.0
// bot state + any URL bookmark + todo direct input
...
module.exports = async function(req, res) {
  if (req.method === "GET") {
    return res.status(200).json({ status: "ok", version: "7.1.0" });
  }
  if (req.method !== "POST") {
...
async function handleQueryTag(replyToken, tag, userId, sheets) {
  if (!tag) { await replyText(replyToken, "\u8ACB\u544A\u8A34\u6211\u8981\u67E5\u8A62\u7684\u6A19\u7C64\uFF0C\u4F8B\u5982\uFF1A\u67E5\u6A19\u7C64 \u65C5\u904A"); return; }
  const data = await getSheetData(sheets, SHEET.THREADS);
...
async function handleFileMessage(replyToken, message, type, userId, clients) {
  ...
  try {
    ...
    if (/Google Drive API has not been used|accessNotConfigured|SERVICE_DISABLED/i.test(msg)) {
      await replyText(replyToken, "Google Drive API \u5C1A\u672A\u555F\u7528\uFF0C\u8ACB\u7B49 5~10 \u5206\u9418\u5F8C\u518D\u6D4B\u8A66\u3002");
      return;
    }
    if (/insufficient.*permission|forbidden|The caller does not have permission/i.test(msg)) {
      await replyText(replyToken, "Google Drive \u6B0A\u9650\u4E0D\u8DB3\uFF0C\u8ACB\u78BA\u8A8D service account \u6B0A\u9650\u3002");
      return;
    }
...
async function uploadToDrive(drive, fileName, mimeType, buffer, fileType) {
  ...
  await drive.permissions.create({ fileId: result.data.id, requestBody: { role: "reader", type: "anyone" } });
  return result.data;
}
