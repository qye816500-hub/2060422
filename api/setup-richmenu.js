...
function lineRequest(method, apiPath, data, contentType) {
  return new Promise(function(resolve, reject) {
    ...
    const req = https.request({ hostname: "api.line.me", path: apiPath, method: method, headers: headers }, function(res) {
      const chunks = [];
...
module.exports = async function(req, res) {
  ...
  const log = [];
  try {
    log.push("Step 1: Deleting existing...");
    ...
    const menuBody = {
      ...
      chatBarText: "\u9078\u55AE",
      areas: [
        { bounds: { x: 0,    y: 0, width: 625, height: 843 }, action: { type: "message", text: "\u5F85\u8FA6\u6E05\u55AE" } },
        { bounds: { x: 625,  y: 0, width: 625, height: 843 }, action: { type: "message", text: "\u6700\u8FD1\u6536\u85CF" } },
        { bounds: { x: 1250, y: 0, width: 625, height: 843 }, action: { type: "message", text: "\u6700\u8FD1\u6536\u85CF" } },
        { bounds: { x: 1875, y: 0, width: 625, height: 843 }, action: { type: "message", text: "\u529F\u80FD\u8AAA\u660E" } }
      ]
    };

    const createRes = await lineRequest("POST", "/v2/bot/richmenu", menuBody);
    log.push("Create status: " + createRes.status);
    log.push("Create body: " + JSON.stringify(createRes.body));
    ...
    return res.status(200).json({ success: true, richMenuId: richMenuId, message: "Rich Menu \u5B8C\u6210\uFF01", log: log });
